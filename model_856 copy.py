...
def add_pf_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # EDA 기반 새 피처
    df["지상역률_보정"] = df["지상역률(%)"].clip(lower=60, upper=95)
    ...
    # 극저 역률 (90% 미만) ← 법적 기준과 동일
    df["극저역률"] = ((df["지상역률_보정"] < 90) & (df["주간여부"] == 1)).astype(int)
...
train["kwh_vs_어제"] = (
    (train["전력사용량(kWh)"] - train["kwh_lag24"]) / (train["kwh_lag24"] + 1e-6)
)
train["전력급등"] = (train["kwh_vs_어제"] > 0.5).astype(int)

train["kwh_roll12_mean"] = train["전력사용량(kWh)"].shift(1).rolling(48).mean()
train["kwh_roll12_std"] = train["전력사용량(kWh)"].shift(1).rolling(48).std().fillna(0)
train["kwh_roll12_cv"] = train["kwh_roll12_std"] / (train["kwh_roll12_mean"] + 1e-6)
train["kwh_roll48_mean"] = train["전력사용량(kWh)"].shift(1).rolling(192).mean()
train["kwh_roll48_std"] = train["전력사용량(kWh)"].shift(1).rolling(192).std().fillna(0)
...
lag1_list, lag24_list, lag96_list, lag168_list = [], [], [], []
r24m, r24s, r96m, r96s = [], [], [], []
lag24_ratio_list, roll24_ratio_list, lag168_ratio_list = [], [], []
vs_yesterday_list = []
roll12_mean_list, roll12_std_list, roll12_cv_list = [], [], []
roll48_mean_list, roll48_std_list, roll48_cv_list = [], [], []
roll24_range_list = []

for y in test["전력사용량(kWh)"].values.astype(float):
    lag1_list.append(hist[-1] if len(hist) >= 1 else np.nan)
    lag24 = hist[-24] if len(hist) >= 24 else np.nan
    lag96_list.append(hist[-96] if len(hist) >= 96 else np.nan)
    lag168 = hist[-168] if len(hist) >= 168 else np.nan

    lag24_list.append(lag24)
    lag168_list.append(lag168)

    arr24 = np.array(hist[-24:]) if len(hist) >= 24 else np.array(hist)
    arr96 = np.array(hist[-96:]) if len(hist) >= 96 else np.array(hist)
    arr12 = np.array(hist[-48:]) if len(hist) >= 48 else np.array(hist)
    arr48 = np.array(hist[-192:]) if len(hist) >= 192 else np.array(hist)
    ...
    lag24_ratio_list.append(y / (lag24 + 1e-6) if not np.isnan(lag24) else np.nan)
    roll24_ratio_list.append(y / (roll24_mean + 1e-6) if not np.isnan(roll24_mean) else np.nan)
    lag168_ratio_list.append(y / (lag168 + 1e-6) if not np.isnan(lag168) else np.nan)
    vs = (y - lag24) / (lag24 + 1e-6) if not np.isnan(lag24) else np.nan
    vs_yesterday_list.append(vs)

    roll12_mean_list.append(roll12_mean)
    roll12_std_list.append(roll12_std)
    roll12_cv_list.append(roll12_std / (roll12_mean + 1e-6) if roll12_mean is not None and not np.isnan(roll12_mean) else np.nan)
    roll48_mean_list.append(roll48_mean)
    roll48_std_list.append(roll48_std)
    roll48_cv_list.append(roll48_std / (roll48_mean + 1e-6) if roll48_mean is not None and not np.isnan(roll48_mean) else np.nan)
    roll24_range_list.append((arr24.max() - arr24.min()) if arr24.size > 0 else np.nan)
...
test["kwh_lag168"] = lag168_list
test["kwh_roll24_mean"] = r24m
test["kwh_roll24_std"] = r24s
test["kwh_roll96_mean"] = r96m
test["kwh_roll96_std"] = r96s
test["kwh_lag24_ratio"] = lag24_ratio_list
test["kwh_roll24_ratio"] = roll24_ratio_list
test["kwh_lag168_ratio"] = lag168_ratio_list
test["kwh_vs_어제"] = vs_yesterday_list
test["전력급등"] = (np.array(vs_yesterday_list) > 0.5).astype(int)
test["kwh_roll12_mean"] = roll12_mean_list
test["kwh_roll12_std"] = roll12_std_list
test["kwh_roll12_cv"] = roll12_cv_list
test["kwh_roll48_mean"] = roll48_mean_list
test["kwh_roll48_std"] = roll48_std_list
test["kwh_roll48_cv"] = roll48_cv_list
test["kwh_roll24_range"] = roll24_range_list
...
def add_advanced_features(df, is_train=True):
    ...
    if "kwh_vs_어제" in df.columns:
        df["kwh_변화율_24h"] = df["kwh_vs_어제"]
    else:
        df["kwh_변화율_24h"] = (
            (df["전력사용량(kWh)"] - df["kwh_lag24"]) / (df["kwh_lag24"] + 1e-6)
        )
    if "전력급등" not in df.columns:
        df["전력급등"] = (df["kwh_변화율_24h"] > 0.5).astype(int)

    if "kwh_roll12_cv" not in df.columns:
        df["kwh_roll12_cv"] = 0.0
    if "kwh_roll48_cv" not in df.columns:
        df["kwh_roll48_cv"] = 0.0
    if "kwh_roll24_range" not in df.columns:
        df["kwh_roll24_range"] = 0.0

    df["위험_변동성"] = df["실질위험"] * df["kwh_roll24_cv"]
...
sample_weights[y_tr > 3000] = 2.0
sample_weights[y_tr > 5000] = 3.0
sample_weights[y_tr > 10000] = 5.0
sample_weights[X_tr["실질위험"].values == 1] *= 2.0

lgb.fit(X_tr, y_tr_log, sample_weight=sample_weights)
xgb.fit(X_tr, y_tr_log, sample_weight=sample_weights)
cat.fit(X_tr, y_tr_log, sample_weight=sample_weights)

pred_lgb_va = np.expm1(lgb.predict(X_va))
pred_xgb_va = np.expm1(xgb.predict(X_va))
pred_cat_va = np.expm1(cat.predict(X_va))
pred_va = (
    0.5 * pred_lgb_va
    + 0.3 * pred_xgb_va
    + 0.2 * pred_cat_va
...
full_weights = np.ones(len(y_all), dtype=float)
full_weights[y_all > 3000] = 2.0
full_weights[y_all > 5000] = 3.0
full_weights[y_all > 10000] = 5.0
full_weights[X_all["실질위험"].values == 1] *= 2.0

lgb_full.fit(X_all, y_all_log, sample_weight=full_weights)
xgb_full.fit(X_all, y_all_log, sample_weight=full_weights)
cat_full.fit(X_all, y_all_log, sample_weight=full_weights)

X_te = test[feat_s2].copy()
pred_lgb_te = np.expm1(lgb_full.predict(X_te))
pred_xgb_te = np.expm1(xgb_full.predict(X_te))
pred_cat_te = np.expm1(cat_full.predict(X_te))
pred_te = w_lgb * pred_lgb_te + w_xgb * pred_xgb_te + w_cat * pred_cat_te
...
# EDA 기반 역률 피처
eda_features = [
    '지상역률_보정', '주간여부', '법적페널티', '실질위험', '극저역률',
    '역률부족폭_94', '역률_60_85', '역률_85_90', '역률_90_94', '역률_94_이상',
...
# Lag/Rolling
lag_features = [
    'kwh_lag1', 'kwh_lag24', 'kwh_lag96', 'kwh_lag168',
    'kwh_roll24_mean', 'kwh_roll24_std',
    'kwh_roll96_mean', 'kwh_roll96_std',
    'kwh_roll12_mean', 'kwh_roll12_std', 'kwh_roll12_cv',
    'kwh_roll48_mean', 'kwh_roll48_std', 'kwh_roll48_cv',
    'kwh_roll24_range'
]
...
mape_mask = y_va_arr != 0
mape = (
    np.mean(np.abs((y_va_arr[mape_mask] - pred_va[mape_mask]) / y_va_arr[mape_mask])) * 100
    if mape_mask.any()
    else np.nan
)
...
# 9) 월별 전체 기간 검증 (1~11월)
# -----------------------------
X_all_eval = train[feat_s2].copy()
train_pred_lgb_full = np.expm1(lgb_full.predict(X_all_eval))
train_pred_xgb_full = np.expm1(xgb_full.predict(X_all_eval))
train_pred_cat_full = np.expm1(cat_full.predict(X_all_eval))
train_pred_full = (
    w_lgb * train_pred_lgb_full
    + w_xgb * train_pred_xgb_full
    + w_cat * train_pred_cat_full
)
...
for month in sorted(train["월"].unique()):
    ...
    if not month_mask.any():
        continue
    month_mae = mean_absolute_error(y_all[month_mask], train_pred_full[month_mask])
    month_r2 = r2_score(y_all[month_mask], train_pred_full[month_mask])
    monthly_stats.append((month, month_mae, month_r2))
    print(f"📅 {month}월 전체 검증: MAE={month_mae:.2f}원 | R²={month_r2:.4f}")

if monthly_stats:
    best_month, best_mae, best_r2 = min(monthly_stats, key=lambda x: x[1])
    print(
        f"\n✅ 월별 MAE 최저: {best_month}월 (MAE={best_mae:.2f}원, R²={best_r2:.4f})"
    )
...
# 극단값 안정화는 유지할지 선택 — 유지하려면 아래 주석 제거
# low, high = np.percentile(pred_te_raw_nocorr, [0.2, 99.8])
# pred_te_raw_nocorr = np.clip(pred_te_raw_nocorr, low, high)

submission_nocorr = pd.DataFrame({
    "id": test["id"],
    "target": pred_te_raw_nocorr
})

submission_nocorr.to_csv("submission_nocorrection.csv", index=False)
print("💾 submission_nocorrection.csv 저장 완료!")
